...
class NextLargerElement {
	...
	static int[] nextLargerElement(int ar[], int n) {
		...
		int[] ans = new int[n];
		for (int i = 0; i < n; i++) {
			// If the incoming element is greater, then it is the next greater element for
		...
		// when all the array elements have been traversed, for the remaining elements
		// in the Stack, we can simply add -1.
		while (!stack.empty()) {
			ans[stack.peek()] = -1;
			stack.pop();
		}
